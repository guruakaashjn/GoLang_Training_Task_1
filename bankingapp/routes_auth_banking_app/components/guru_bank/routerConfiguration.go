...
func HandleRouter(parentRouter *mux.Router) {
	...
	bankRouter.HandleFunc("/{bank-id}/networth", bank_controller.NetWorthEachBank).Methods("GET")
	bankRouter.HandleFunc("/{bank-id}/bank-balance-map", bank_controller.BankNameBalanceMapByBankId).Methods("POST")
	bankRouter.HandleFunc("/{bank-id}/all-bank-balance-map", bank_controller.BankNameBalanceMapAll).Methods("POST")
}
